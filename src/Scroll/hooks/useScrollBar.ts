...
function useScrollBar(options: UseScrollBarOptions): UseScrollBarResult {
    ...
    const onThumbMove = (moveEvent: any) => {
        ...
        if (barType == 'y' && scrollElementRef.current) {
            ...
            const newScrollTop = getScrollTopByThumbOffset(
                scrollElementRef.current,
                newThumbOffset,
            );
    ...
    return { thumbClient, thumbOffset, trackClient, updateTrackClient, updateThumbClient, updateThumbOffset, trackRef, thumbRef }
}
